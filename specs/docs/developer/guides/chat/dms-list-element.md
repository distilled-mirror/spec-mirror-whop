...
Filter the DMs list to only show conversations belonging to a specific account by passing a `companyId`. To programmatically create DMs, use the [Create DM Channel](/api-reference/dm-channels/create-dm-channel) endpoint, passing the same `companyId` so the DM appears in the scoped list.
