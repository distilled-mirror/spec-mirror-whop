...
<div data-whop-platform="react-native" style={{ display: "none" }}>
  ...
  `appearance` on `<WhopElements>` themes every element beneath it, with the same `theme` object the web elements take. See [Appearance](/elements/beta/appearance).
  ...
  * [`PaymentElement`](/elements/beta/payments/payment): the method tiles and everything the selected method needs
  * [`AddressElement`](/elements/beta/payments/address): the billing address in the buyer's own country format
  * [`CardElement`](/elements/beta/payments/card): number, expiry and security code as one component
  * [`CardFields`](/elements/beta/payments/cardFields): the same three fields, placed individually
  * [`EmailElement`](/elements/beta/payments/email): the buyer's email
  * [`TaxIdElement`](/elements/beta/payments/taxId): a registration type and number
  * [`BrandingElement`](/elements/beta/payments/branding): the Powered by Whop mark, required on every form

  The checkout elements:

  * [`ExpressCheckoutElement`](/elements/beta/checkout/expressCheckout): one Apple Pay or Google Pay press that pays a whole checkout, under `<Checkout>`
</div>
