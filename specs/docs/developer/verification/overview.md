...
<Note>
  You need an Account API key with the `identity:write` scope. See [Auth & API keys](/developer/guides/auth-scoping) for the credential types and how to scope them. Go to your [dashboard](https://whop.com/dashboard) → Settings → API Keys to create one.
</Note>
