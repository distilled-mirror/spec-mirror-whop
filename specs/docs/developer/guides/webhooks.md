...
For a failed subscription renewal, handle `payment.failed` and inspect `data.decline_code` and `data.failure_message` for the reason when available. If `data.recovery_url` is present, send that link to the buyer so they can sign in to Whop and complete 3D Secure verification for that payment.
