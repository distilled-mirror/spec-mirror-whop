...
| `partner_payouts`  | `currency` | `status`, `account_id`, `tier`. `referred_user_id` is filter-only | Completed commissions from referred businesses, bucketed by payout date. |
