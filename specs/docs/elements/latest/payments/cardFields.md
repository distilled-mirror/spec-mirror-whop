...
<div data-whop-platform="web">
  Mounts inside [`Payments`](/elements/latest/payments/overview). Create it to get a handle, then mount its elements on that handle. Call `destroy()` to remove the sub-controller and free its slot. Create it again to get a fresh handle.
  ...
  <div data-whop-demo-shell style={{ position: "relative", minHeight: "480px", transition: "min-height 200ms ease" }}>
    ...
    <div data-whop-demo-native="unit:card-fields" data-whop-elements-version="1.1.0" style={{ position: "relative" }} />
  </div>
</div>

<div data-whop-platform="swift" style={{ display: "none" }}>
  Mounts inside a `WhopPayments` scope. The same three hosted inputs as [`WhopCardElement`](/elements/latest/payments/card#swift), placed one by one inside its content builder so your own layout decides where each sits.
</div>
...
<CardGroup cols={2}>
  <Card title="CardNumberElement" href="/elements/latest/payments/cardFields-cardNumber">
    PCI-isolated hosted card number field. Card numbers never reach the host page.
  </Card>

  <Card title="CardExpiryElement" href="/elements/latest/payments/cardFields-cardExpiry">
    PCI-isolated hosted card expiration field.
  </Card>

  <Card title="CardCvcElement" href="/elements/latest/payments/cardFields-cardCvc">
