...
<div data-whop-platform="web">
  Mounts inside [`Wallet`](/elements/beta/wallet/overview). `accountId` and `accessToken` come from there. Pass props and callbacks through the create options or React props. Keep the created handle, or React `ref`, to call `refresh()` and `setDateRange()`.
</div>
